...
def print_record_status(domain, records, record_type=None, name=None):
    ...
    if not found:
        print(
            f"  [MISSING] No matching record found for type={record_type or 'ANY'}, name={name or 'ANY'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--domain", help="Domain to check (default: all domains in account or DO_APP_DOMAIN)"
    )
    ...
    parser.add_argument(
        "--check-required",
        action="store_true",
        help="Check required app records (@, www, traefik, pgadmin, django-admin, flower)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit records as JSON (machine-readable; no human formatting)",
    )
