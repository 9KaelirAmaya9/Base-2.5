...
try:
    ...
    print(
        "[ERROR] pydo not installed. Please run 'pip install pydo' in your environment.",
        file=sys.stderr,
    )
    sys.exit(1)
...
try:
    ...
    def log_exec_start(*a, **kw):
        pass

    def log_exec_success(*a, **kw):
        pass

    def log_exec_error(*a, **kw):
        pass
...
def exec_on_app_service(client, app_id, service_name, command):
    ...
    print(
        "[INFO] Digital Ocean App Platform exec is not yet supported via PyDo. Use 'doctl' CLI or dashboard."
    )
    log_exec_error(
        f"app:{app_id}:{service_name}", command, "API not supported; use doctl/dashboard"
    )
    return 127
...
def main():
    ...
    try:
        if args.droplet:
            ...
            sys.exit(2)
        log_exec_success(
            args.droplet or f"app:{args.app}:{args.service}", args.cmd, f"exit_code={rc}"
        )
        sys.exit(rc)
