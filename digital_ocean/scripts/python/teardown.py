        ...
        try:
            ...
            try:
                ...
                if "rate limit" in str(delete_err).lower() and attempt < max_retries:
                    ...
                    continue
                logger.error(
                    f"Rollback failed: Could not delete droplet {droplet_id}: {delete_err}"
                )
                logger.critical("Teardown rollback triggered due to deletion failure.")
                print(
                    f"[ROLLBACK ERROR] Could not delete droplet {droplet_id}: {delete_err}",
                    file=sys.stderr,
                )
                sys.exit(4)
            break
