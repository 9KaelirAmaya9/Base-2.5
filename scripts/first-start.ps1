...
.DESCRIPTION
    ...
    1) scripts/bootstrap-venv.ps1 (create or recreate .venv)
    2) Activate .venv for the current PowerShell session
    3) scripts/install-python-deps.ps1 (Python deps for automation)
    4) scripts/install-node-deps.ps1 (Node deps for root/react-app/e2e)
    5) scripts/setup.ps1 (guided .env generation from .env.example)
...
.PARAMETER SkipSetup
    Skip running scripts/setup.ps1. Use this if you only need dependency hydration.

.SETUP_OPTIONS
    scripts/setup.ps1 supports additional options when you need fine control.
...
.EXAMPLE
    ./scripts/first-start.ps1 -ForceVenv

.EXAMPLE
    ./scripts/first-start.ps1 -SkipSetup

.EXAMPLE
    # Run first-start, then run setup with extra options
    ./scripts/first-start.ps1 -SkipSetup
    ./scripts/setup.ps1 -NonInteractive -EnvPath .\.env.staging -DoSyncDryRun
...
if ($Help) {
    Write-Host 'Usage: ./scripts/first-start.ps1 [options]' -ForegroundColor Cyan
    Write-Host ''
    Write-Host 'Options:'
    Write-Host '  -ForceVenv   Recreate the .venv even if one already exists'
    Write-Host '  -SkipSetup  Skip running scripts/setup.ps1'
    Write-Host '  -Help, -h   Show this help'
    Write-Host ''
    Write-Host 'Tip: run ./scripts/setup.ps1 -Help for setup options.'
    return
}
