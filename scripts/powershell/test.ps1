...
function Require-ComposeRunning {
    ...
    if (-not $docker) {
        throw 'Docker is not installed or not on PATH. Run ./scripts/powershell/start.ps1 first.'
    }
    ...
    if (-not $running -or ($running -notcontains 'api')) {
        throw 'api container is not running. Run ./scripts/powershell/start.ps1 (or make up) first.'
    }
    if ($running -notcontains 'django') {
        throw 'django container is not running. Run ./scripts/powershell/start.ps1 (or make up) first.'
    }
}
