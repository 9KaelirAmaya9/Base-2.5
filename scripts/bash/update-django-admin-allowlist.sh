...
usage() {
  cat <<'EOF'
Usage: ./scripts/bash/update-django-admin-allowlist.sh [options]
...
if $DRY_RUN; then
  echo "[DryRun] Would write DJANGO_ADMIN_ALLOWLIST=$cidr"
  exit 0
fi

tmp_file=$(mktemp)
if [[ -f "$ENV_PATH" ]]; then
  grep -v '^DJANGO_ADMIN_ALLOWLIST=' "$ENV_PATH" > "$tmp_file" || true
fi
printf 'DJANGO_ADMIN_ALLOWLIST=%s\n' "$cidr" >> "$tmp_file"
mv "$tmp_file" "$ENV_PATH"

if grep -q '^DJANGO_ADMIN_ALLOWLIST=' "$ENV_PATH"; then
  echo "Updated: DJANGO_ADMIN_ALLOWLIST=$cidr"
  echo "Done. Restart Traefik to apply: docker compose up -d --force-recreate traefik"
else
  echo "Failed to confirm DJANGO_ADMIN_ALLOWLIST write." >&2
  exit 1
fi
