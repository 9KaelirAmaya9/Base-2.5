...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --env-file|-e)
            ...
            ;;
        --help|-h)
            echo "Usage: ./scripts/bash/sync-env.sh [OPTIONS]"
            echo ""
            echo "Options:"
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    if [ -f .env.example ]; then
        ...
        echo "âŒ Error: .env.example not found. Cannot create $ENV_FILE."
        exit 1
    fi
fi
...
echo -e "${BLUE}ðŸ”„ Synchronizing configuration with .env variables...${NC}"
...
if [ -z "$NETWORK_NAME" ]; then
    echo "âŒ Error: NETWORK_NAME not set in .env"
    exit 1
fi
...
# ============================================
echo "ðŸ” Checking .env consistency..."
...
if [ "$CURRENT_TRAEFIK_NETWORK" != "$NETWORK_NAME" ]; then
    if grep -q "^TRAEFIK_DOCKER_NETWORK=" "$ENV_FILE"; then
        ...
        echo -e "${YELLOW}âš™ï¸  Setting TRAEFIK_DOCKER_NETWORK: (missing) â†’ $NETWORK_NAME${NC}"
...
# ============================================
echo "ðŸ” Checking compose network configuration in $COMPOSE_FILE..."
...
if [ -z "$CURRENT_NETWORK" ]; then
    echo -e "${YELLOW}âš ï¸  Warning: Could not detect a networks key in $COMPOSE_FILE${NC}"
elif [ "$CURRENT_NETWORK" != "$TARGET_NETWORK_KEY" ]; then
    echo -e "${YELLOW}âš™ï¸  Updating network key: $CURRENT_NETWORK â†’ $TARGET_NETWORK_KEY${NC}"
    ...
    echo -e "${GREEN}âœ… Updated compose network key${NC}"
    CHANGES_MADE=true

    rm -f "$COMPOSE_FILE.bak"
else
    echo "âœ“ compose network key is correct ($TARGET_NETWORK_KEY)"
fi
...
echo "âœ“ compose network name is driven by NETWORK_NAME"
echo ""
...
normalize_basic_users() {
    ...
    if grep -q "^${key}=" "$ENV_FILE"; then
        ...
        if [ "$original_line" != "$sanitized_line" ]; then
            echo -e "${YELLOW}âš™ï¸  Normalizing ${key} dollar signs for Compose compatibility${NC}"
...
if [ "$CHANGES_MADE" = true ]; then
    echo -e "${GREEN}ðŸŽ‰ Synchronization complete! Changes were made.${NC}"
    echo ""
    echo "Current configuration:"
    echo "  - Network Name: $NETWORK_NAME"
    echo ""
    echo "ðŸ’¡ You should rebuild your containers for changes to take effect:"
    echo "   ./scripts/bash/stop.sh && ./scripts/bash/start.sh --build"
else
