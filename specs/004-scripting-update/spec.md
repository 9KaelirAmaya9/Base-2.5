...
| ------------ | ------------------------------------ | ------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------ | ---------------------------------------------------------------------------- |
| setup        | scripts/bash/setup.sh                | scripts/powershell/setup.ps1                | N                                                      | Bash: forwards args to `scripts/setup.js`. PS: `-NonInteractive`, `-SkipSetupJs`, `-EnvPath`, `-DoSyncDryRun`, `-Help`, extra args after `--`.                           | 0 success; 1+ failure; 127 if node missing | Generates `.env`, secrets, and DO SSH sync.                                  |
| start        | scripts/bash/start.sh                | scripts/powershell/start.ps1                | N                                                      | Bash: `-c/--compose-file`, `-e/--env-file`, `-b/--build`, `-f/--foreground`, `--self-test`, `--follow-logs`, `-h/--help`. PS: `-ComposeFile`, `-EnvFile`, `-Build`.      | 0 success; 1+ failure                      | Requires Docker Compose v2; creates `.env` from `.env.example` when missing. |
| stop         | scripts/bash/stop.sh                 | scripts/powershell/stop.ps1                 | N                                                      | Bash: `-c/--compose-file`, `-e/--env-file`, `-v/--volumes`, `--self-test`, `-h/--help`. PS: `-ComposeFile`, `-EnvFile`.                                                  | 0 success; 1+ failure                      | `--volumes` prompts and removes data.                                        |
| restart      | scripts/bash/restart.sh              | scripts/powershell/restart.ps1              | N                                                      | Bash: `[SERVICE]`, `-c/--compose-file`, `-e/--env-file`, `-h/--help`. PS: `-Build`.                                                                                      | 0 success; 1+ failure                      | PS restart calls stop/start.                                                 |
| logs         | scripts/bash/logs.sh                 | scripts/powershell/logs.ps1                 | N                                                      | Bash: `[SERVICE]`, `-c/--compose-file`, `-e/--env-file`, `-f/--follow`, `-t/--tail`, `--self-test`, `-h/--help`. PS: `-Services`, `-Tail`, `-ComposeFile`, `-EnvFile`.   | 0 success; 1+ failure                      |                                                                              |
| test         | scripts/bash/test.sh                 | scripts/powershell/test.ps1                 | N                                                      | Bash: `-c/--compose-file`, `-e/--env-file`, `--coverage`, `--watch`, `--self-test`, `-h/--help`. PS: `-ComposeFile`, `-EnvFile`.                                         | 0 success; 1+ failure                      | Requires compose services running.                                           |
| deploy       | digital_ocean/scripts/bash/deploy.sh | digital_ocean/scripts/powershell/deploy.ps1 | Y (digital_ocean/scripts/python/orchestrate_deploy.py) | PS: `-Full`, `-UpdateOnly`, `-AllTests`, `-RunAllTests`, `-Timestamped`, `-LogsDir`, `-Preflight`, `-RunTests`, `-TestsJson`, `-Help`. Bash: mirror MVP flags from docs. | 0 success; 1+ failure                      | Remote payload allowed via designated remote-exec helper.                    |
