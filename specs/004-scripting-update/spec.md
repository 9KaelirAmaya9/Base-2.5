...
| deploy       | digital_ocean/scripts/bash/deploy.sh | digital_ocean/scripts/powershell/deploy.ps1 | Y (digital_ocean/orchestrate_deploy.py)        | PS: `-Full`, `-UpdateOnly`, `-AllTests`, `-RunAllTests`, `-Timestamped`, `-LogsDir`, `-Preflight`, `-RunTests`, `-TestsJson`, `-Help`. Bash: mirror MVP flags from docs. | 0 success; 1+ failure                      | Remote payload allowed via designated remote-exec helper.                    |
