...
def _require_cookie_csrf(request: Request) -> None:
    ...
    header_val = (
        request.headers.get('x-csrf-token') or request.headers.get('X-CSRF-Token') or ''
    ).strip()
        ...
        if str(e) == 'invalid_password':
            raise HTTPException(
                ...
                detail=[
                    {
                        'loc': ['body', 'password'],
                        'msg': 'Password does not meet policy',
                        'type': 'value_error',
                    }
                ],
            ) from e
...
async def auth_forgot_password(request: Request, response: Response):
    ...
    try:
        ...
        if normalized:
            ...
            _email_count, email_over, email_retry_after = incr_and_check_identifier_detailed(
                email_hash, 'forgot_password_email'
            )
            if email_over:
                raise HTTPException(
                    status_code=429,
...
async def auth_oauth_google(request: Request, response: Response):
    ...
    try:
        ...
        ident = verify_google_id_token(
            id_token=str(id_token), audience=str(settings.GOOGLE_OAUTH_CLIENT_ID)
        )
    except ValueError as e:
