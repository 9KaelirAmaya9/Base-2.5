...
def register_user(
    *,
    ...
    return user, AuthTokens(
        access_token=access, refresh_token=refresh, refresh_token_expires_at=refresh_expires_at
    )
...
def login_user(
    *,
    ...
    if not user.is_active:
        repo.insert_audit_event(
            user_id=user.id, action='auth.login_failed', ip=ip, user_agent=user_agent
        )
        raise ValueError('invalid_credentials')
    ...
    try:
        ...
        if locked_until is not None:
            now = (
                datetime.now(locked_until.tzinfo)
                if getattr(locked_until, 'tzinfo', None)
                else datetime.utcnow()
            )
            if locked_until > now:
                repo.insert_audit_event(
                    user_id=user.id, action='auth.login_locked', ip=ip, user_agent=user_agent
                )
                raise ValueError('invalid_credentials')
    except ValueError:
        raise
    ...
    if not verify_password(password, user.password_hash):
        ...
        try:
            ...
            st = repo.register_login_failure(
                user_id=user.id, max_failures=max_failures, lock_minutes=lock_minutes
            )
            if (
                st.get('locked_until') is not None
                and int(st.get('failed_login_attempts') or 0) >= max_failures
            ):
                repo.insert_audit_event(
                    user_id=user.id, action='auth.account_locked', ip=ip, user_agent=user_agent
                )
        except Exception:
...
def refresh_tokens(
    *, refresh_token: str, ip: str, user_agent: str, refresh_ttl_days: int, access_ttl_minutes: int
) -> tuple[repo.User, AuthTokens]:
    ...
    if rec['revoked_at'] is not None:
        ...
        repo.insert_audit_event(
            user_id=rec['user_id'], action='auth.refresh_reuse', ip=ip, user_agent=user_agent
        )
        raise ValueError('invalid_refresh')
    ...
    return user, AuthTokens(
        access_token=access, refresh_token=new_refresh, refresh_token_expires_at=new_expires_at
    )
